...
1. Consider a model like:
    ...
    a. The relationship between $\text{corr} (X_1, X_2)$ and $\widehat{\se(\hat{\beta}_3)}$.
...
1. I collect a set of data ($N = 100$ observations) containing a single predictor and a quantitative response. I then fit a linear regression model to the data

    $$Y = \beta_0 + \beta_1 X + \epsilon$$
    
    as well as a separate cubic regression
    
    $$Y = \beta_0 + \beta_1 X + \beta_2 X^2 + \beta_3 X^3 + \epsilon$$
    
    a. Suppose that the true relationship between $X$ and $Y$ is in fact linear. Consider the training residual sum of squares (RSS) for the linear regression, and also the training RSS for the cubic regression. Would we expect one to be lower than the other, would we expect them to be the same, or is there not enough information to tell? Justify your answer.
    a. Answer (a) using test rather than training RSS.
    ...
    a. Answer (c) using test rather than training RSS.
