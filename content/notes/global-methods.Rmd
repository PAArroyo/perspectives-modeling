...
# estimate the predicted values and confidence interval
mh_pred <- augment(mh_mod, newdata = mh) %>%
  distinct(mhealth_sum, .fitted, .se.fit) %>%
  rename(pred = .fitted) %>%
...
mh_pred %>%
  mutate_at(vars(starts_with("pred")), funs(logit2prob)) %>%
...
# estimate the predicted values and confidence interval
biden_step_pred <- augment(biden_step, newdata = biden) %>%
  mutate(pred_low = .fitted - 1.96 * .se.fit,
         pred_high = .fitted + 1.96 * .se.fit)
...
# estimate the predicted values and confidence interval
biden_spline_pred <- augment(biden_spline, newdata = biden) %>%
  mutate(pred_low = .fitted - 1.96 * .se.fit,
         pred_high = .fitted + 1.96 * .se.fit)
...
But this still leaves unaddressed the matter of how many knots should we use? Or how many degrees should each polynomial be? While theory should still be our guide, we can also use [cross-validation](/notes/cross-validation/) to determine the optimal number of knots and/or polynomial degrees.
...
wage_spline <- function(splits, degree = 3, df = NULL){
  # estimate the model on each fold
  model <- glm(wage ~ bs(age, df = df, degree = degree),
...
tune_over_degrees <- function(splits){
  results <- tibble(
    degree = 1:10,
    df = 3 + degree
  ) %>%
    mutate(mse = map2_dbl(degree, df, ~ wage_spline(splits, .x, .y)))
}

summarize_tune_results <- function(splits){
  map_df(splits$splits, tune_over_degrees) %>%
    group_by(degree) %>%
    summarize(mean_mse = mean(mse))
}

# estimate mse for polynomial degrees in 1:10
results <- nested_cv(wage,
                     outside = vfold_cv(v = 10),
                     inside = vfold_cv(v = 10)) %>%
  mutate(degrees = map(inner_resamples, summarize_tune_results))

# graph the results
results %>%
  unnest(degrees) %>%
  group_by(degree) %>%
  summarize(mse = mean(mean_mse)) %>%
...
tune_over_df <- function(splits){
  results <- tibble(
    degree = 1:10,
    df = 3 + degree
  ) %>%
    mutate(mse = map_dbl(df, ~ wage_spline(splits, df = .x)))
}

summarize_tune_results_df <- function(splits){
  map_df(splits$splits, tune_over_df) %>%
    group_by(df) %>%
    summarize(mean_mse = mean(mse))
}

results <- results %>%
  mutate(df = map(inner_resamples, summarize_tune_results_df))
...
These results suggest the optimal number of polynomial degrees is 2, while the optimal number of knots is 2. The resulting model produced by these parameters is:

```{r wage-optimal-mod}
wage_optim <- glm(wage ~ bs(age, df = 2, degree = 2), data = wage)

augment(wage_optim, newdata = wage) %>%
  ...
  geom_vline(xintercept = attr(bs(wage$age, df = 8, degree = 6), "knots"),
             linetype = 2, color = "blue") +
  labs(title = "Second-order polynomial spline of wage",
       subtitle = "Knots = 2",
       x = "Age",
       y = "Wage")
```
