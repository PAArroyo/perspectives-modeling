...
This method produces estimates of the error rate that are approximately unbiased and are non-varying for a given dataset, unlike the validation set approach where the MSE estimate is highly dependent on the sampling process for training/test sets. However it can have high variance because the $N$ "training sets" are so similar to one another. LOOCV is also highly flexible and works with any kind of predictive modeling.
