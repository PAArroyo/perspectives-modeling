...
The logistic transformation produces an S-shaped curve that preserves ordering; that is, larger values of $X$ will also result in larger values of $\Pr(Y)$, but the relationship is now **curvilinear**. $\Pr(Y)$ will never decrease below 0 and never exceed 1, so the predicted probabilities will always have inherent meaning.
...
$$\Pr(\text{Survival}) = \frac{e^{`r coef(survive_age)[[1]]` + `r coef(survive_age)[[2]]` \times 30}}{1 + e^{`r coef(survive_age)[[1]]` + `r coef(survive_age)[[2]]` \times 30}}$$

```{r prob-30, include = FALSE}
prob30 <- predict(survive_age, newdata = tibble(Age = 30), type = "response")
```

$$\Pr(\text{Survival}) = `r formatC(prob30[[1]], digits = 3)`$$
...
$$\Pr(X) = \frac{e^{\beta_0 + \beta_{1}X_1 + \dots + \beta_{p}X_{p}}}{1 + e^{\beta_0 + \beta_{1}X_1 + \dots + \beta_{p}X_{p}}}$$
...
$$\Pr(X) = \frac{e^{\beta_0 + \beta_{1}X_1 + \beta_{2}X_2}}{1 + e^{\beta_0 + \beta_{1}X_1 + \beta_{2}X_2}}$$
...
$$\Pr(\text{Survival}_{male}) = \frac{e^{\beta_0 + (\beta_{1} + \beta_{3})\text{Age}}}{1 + e^{\beta_0 + (\beta_{1} + \beta_{3})\text{Age}}}$$
...
This model is much better. Just by knowing an individual's age and gender, we can predict with `r formatC(mean(x_accuracy$Survived != x_accuracy$pred, na.rm = TRUE) * 100, digits = 3)`% error whether he/she lives or dies.
