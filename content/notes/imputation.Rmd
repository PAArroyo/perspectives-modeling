...
**Conditional-mean imputation** replaces missing data with predicted values obtained from a statistical learning model. Using the available data, predict each variable with missing data using the other variables in the data set. Regression is a simple approach, though more complex machine learning algorithms can be used. However this still leaves two problems:
...
Many of the following strategies can be implemented using the [`recipes`](https://tidymodels.github.io/recipes/index.html) package in R. Some of the most basic methods don't even require special packages (e.g. mean/mode imputation) but `recipes` is a recently developed tool that makes creating and pre-processing design matrices for statistical modeling far easier than previous methods. Likewise, many of these methods have analogous implementations in Python.
...
We will demonstrate these methods using the [`credit_data` dataset](https://tidymodels.github.io/recipes/reference/credit_data.html), which is a sample of individuals with credit scores and related data.
...
### Modal imputation for nominal data
...
Alternatively, we can use a tree-based method to pursue a non-parametric imputation strategy. In this approach, you build an ensemble of decision trees for each variable to predict its value using the other variables in the dataset. These results are used to generate imputed values for all the missing variables and observations.

Any tree aggregation method can be employed (e.g. bagging, random forest, boosting). `recipes` uses the bagging method:
...
## Estimation strategies for multiple imputation
...
Let $\Pr(\mathbf{X}, \theta) = \Pr(\mathbf{X}_{\text{obs}}, \mathbf{X}_{\text{mis}}; \theta)$ represent the joint probability density for the complete data $\mathbf{X}$, which is composed of the observed and missing components denoted by $\mathbf{X}_{\text{obs}}, \mathbf{X}_{\text{mis}}$. The vector $\theta$ contains the unknown parameters on which the complete-data distribution depends. For example, if the variables in $\mathbf{X}$ are multivariate normally distributed, then $\theta$ includes the population means and covariances among the variables.
...
$\se^2(B_j^{(l)})$ is the standard error of $B_j$, computed in the usual manner for the $l$th imputed dataset.

Inference based on $\tilde{\se}(\tilde{\beta}_j)$ and $\tilde{\se}(\tilde{\beta}_j)$ follows the $t$-distribution with degrees of freedom:

$$df_j = (g-1) \left ( 1 + \frac{g}{g+1} \times \frac{\Var_j^{(W)}}{\Var_j^{(B)}} \right)^2$$
